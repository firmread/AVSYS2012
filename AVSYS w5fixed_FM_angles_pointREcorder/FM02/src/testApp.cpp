...
void testApp::setup(){	
	...
	panel.addSlider("modulator amount", "VAL_MODULATOR_AMOUNT", 10,0, 800, false);			//Amount/range it will modulate by
...
void testApp::update(){
	...
	lowFreq					= MAX(panel.getValueF("VAL_CARRIER_FREQ") - panel.getValueF("VAL_MODULATOR_AMOUNT"), 0) ;
	highFreq				= MIN(panel.getValueF("VAL_CARRIER_FREQ") + panel.getValueF("VAL_MODULATOR_AMOUNT"), 44100.0/2) ;
